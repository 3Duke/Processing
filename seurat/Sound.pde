...
class Sound {
  ...
  void play() {
    ...
    int effectiveFrameRate = 20; // + 4*int(abs(sin(TWO_PI*(frameCount/1000)))); 
    ...
    float pulseWidth2 = map( frameSet.frame[0].particles[0].radius, frameSet.minRadius, frameSet.maxRadius, 0.05, 0.2 );
    ...
    if (frameCount % effectiveFrameRate == 0) {
      ...
      if (frameCount > frameSet.frame[1].phase) {
        println("2");
        pulse(freq3, 0, 0.4, 5);
        pulse(freq4, 0, pulseWidth4, 5.20);
        pulse(freq3, 0, 0.4, 10);
        pulse(freq4, 0, pulseWidth4, 10.20);
      }
    }  // play
  }
